...
func replaceDocs(content string, originalFile string) string {
	...
	content = re.ReplaceAllString(content, "Original file: "+originalRelative)

	reBuild := regexp.MustCompile("// .*build generic" + config.App.LineSeparator)
	content = reBuild.ReplaceAllString(content, "")
